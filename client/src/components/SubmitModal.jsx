...
export default function SubmitModal({ open, onClose }) {
  ...
  async function handleSubmit() {
    ...
    if (tab === 'live' && !form.time) return setError('Please select the time for the live class.')
    ...
    try {
      await submitLink({
        ...
        tute_url: form.tute_url || '', date: form.date || new Date().toLocaleDateString('en-CA'),
          ...
          {tab === 'live' && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <FormField label="Date" required hint="Shows 'Today' if today">
                <Input type="date" value={form.date || new Date().toLocaleDateString('en-CA')} onChange={e => set('date', e.target.value)} />
              </FormField>
