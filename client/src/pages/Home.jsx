    ...
    <div style={{ minHeight:'100vh' }}>
      ...
      <nav style={{ position:'sticky', top:0, zIndex:200, background:'rgba(9,9,15,0.96)', backdropFilter:'blur(24px)', borderBottom:'1px solid var(--b1)' }}>
        <div style={{ maxWidth:1500, margin:'0 auto', padding:'0 24px', height:62, display:'flex', alignItems:'center', gap:16 }}>
          ...
          <div className="desktop-nav-tabs" style={{ display:'flex', gap:2, marginLeft:8, position:'relative' }}>
            <TabBtn active={tab==='videos'} onClick={()=>setTab('videos')} icon={<VideoIcon size={14}/>} label="Videos"/>
            <TabBtn active={tab==='live'} onClick={()=>setTab('live')} icon={<RadioIcon size={14}/>} label="Live" badge={live.length}/>
            <TabBtn active={tab==='materials'} onClick={()=>setTab('materials')} icon={<FolderIcon size={14}/>} label="Materials"/>
          </div>
      ...
      <div className="mobile-top-bar" style={{ display:'none', position:'sticky', top:0, zIndex:200, background:'rgba(9,9,15,0.96)', backdropFilter:'blur(24px)', borderBottom:'1px solid var(--b1)' }}>
        <a href="/" style={{ textDecoration:'none', flexShrink:0 }}>
          <div style={{ width:34, height:34, borderRadius:9, background:'#0d0d1f', border:'1.5px solid #1e1b4b', display:'flex', alignItems:'center', justifyContent:'center', position:'relative', overflow:'hidden' }}>
            <span style={{ position:'absolute', fontFamily:"'Inter',sans-serif", fontSize:28, fontWeight:900, color:'#4338ca', opacity:0.22, right:-3, bottom:-5, lineHeight:1 }}>S</span>
            <span style={{ fontFamily:"'Noto Sans Sinhala','Iskoola Pota',serif", fontSize:18, fontWeight:700, position:'relative', zIndex:2, background:'linear-gradient(135deg,#a5b4fc,#c4b5fd)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', paddingBottom:2 }}>ගු</span>
          </div>
        </a>
        {searchInput}
      </div>
      ...
      {/* ─── HERO ─── */}
      <div style={{ background:'linear-gradient(180deg,var(--s1) 0%,var(--bg) 100%)', borderBottom:'1px solid var(--b1)', padding:'20px 24px 16px' }}>
