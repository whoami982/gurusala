...
router.post('/submit', submitLimiter, async (req, res) => {
  ...
  if (type === 'live' && !time) return res.status(400).json({ error: 'Time is required for live class.' });
  ...
  // Validate date if provided - allow today and future
  if (date) {
    // Parse in local timezone by appending time
    const parts = date.split('-');
    if (parts.length === 3) {
      const d = new Date(parseInt(parts[0]), parseInt(parts[1])-1, parseInt(parts[2]));
      const today = new Date(); today.setHours(0,0,0,0);
      const maxDate = new Date(); maxDate.setFullYear(maxDate.getFullYear()+1);
      if (d < today) return res.status(400).json({ error: 'Date cannot be in the past.' });
      if (d > maxDate) return res.status(400).json({ error: 'Date too far in future.' });
    }
  }
